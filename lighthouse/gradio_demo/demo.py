...
WEIGHTS_DIR = "/root/spill/weights"
...
def load_pretrained_weights():
    ...
    for file_url in tqdm(file_urls):
        if not os.path.exists(os.path.join(WEIGHTS_DIR, os.path.basename(file_url))):
            command = 'wget -P {} {}'.format(WEIGHTS_DIR, file_url)
            subprocess.run(command, shell=True)

    # Slowfast weights
    if not os.path.exists(os.path.join(WEIGHTS_DIR, 'SLOWFAST_8x8_R50.pkl')):
        subprocess.run('wget -P {} https://dl.fbaipublicfiles.com/pyslowfast/model_zoo/kinetics400/SLOWFAST_8x8_R50.pkl'.format(WEIGHTS_DIR), shell=True)

    # PANNs weights
    if not os.path.exists(os.path.join(WEIGHTS_DIR, 'Cnn14_mAP=0.431.pth')):
        subprocess.run('wget -P {} https://zenodo.org/record/3987831/files/Cnn14_mAP%3D0.431.pth'.format(WEIGHTS_DIR), shell=True)
...
model = CGDETRPredictor(os.path.join(WEIGHTS_DIR, 'clip_cg_detr_qvhighlight.ckpt'), device=device, 
...
def model_load(radio, video):
    ...
    if radio is not None:
        ...
        model = model_class(os.path.join(WEIGHTS_DIR, '{}_{}_qvhighlight.ckpt'.format(feature, model_name)),
                            device=device, feature_name=feature,
                            slowfast_path=os.path.join(WEIGHTS_DIR, 'SLOWFAST_8x8_R50.pkl'), 
                            pann_path=os.path.join(WEIGHTS_DIR, 'Cnn14_mAP=0.431.pth'))
